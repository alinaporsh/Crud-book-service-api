   ...
   @Autowired
    ...
    public ResponseEntity<Book> updateBook(@PathVariable Long id, @RequestBody Book updatedBook) {
        try {
            ...
            return ResponseEntity.notFound().build(); // Handle the case where the book is not found
        }
    }
    ...
    public ResponseEntity<String> addNewBook(@RequestBody Book book) {
        try {
            ...
            Long bookId = addedBook.getId(); // Assuming getId() returns the book ID
            return ResponseEntity.status(HttpStatus.CREATED).body("Book added successfully. Book ID: " + bookId);
